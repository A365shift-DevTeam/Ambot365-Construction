    ...
    <section id="about" className="bg-white border-b border-[#E6E4DE]">
      ...
      <div className="border-b border-[#E6E4DE]">
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-y-9 py-11">
            {stats.map((stat, i) => (
              <div key={i} className="flex flex-col border-l border-[#E6E4DE] pl-6 first:border-l-0 first:pl-0">
                <div className="font-display font-semibold text-[42px] md:text-6xl tracking-[-1.8px] text-[#0B111F] tabular-nums flex items-baseline">
                  {stat.value}
                  <span className="text-[22px] md:text-[27px] ml-1 text-[#B87333] font-medium tracking-normal">{stat.unit}</span>
                </div>
                <div className="mt-1 uppercase text-xs tracking-[0.2em] text-[#0B111F]/50 font-medium">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 pt-16 pb-20">
        <div className="max-w-4xl">
          <div className="uppercase text-[#B87333] tracking-[0.2em] text-xs font-medium mb-4">EST. 2001</div>
          <h2 className="display-lg text-6xl md:text-[68px] tracking-[-3.6px] leading-none mb-6">
            Engineering for<br />the long term.
          </h2>
          <p className="text-xl text-[#0B111F]/70 max-w-3xl leading-snug">
          ...
          {pillars.map((p, idx) => {
            ...
            return (
              <div
                ...
                className={`enterprise-card group rounded-3xl p-9 flex flex-col bg-white cursor-default ${active ? 'border-[#B87333]/40 shadow-xl' : ''}`}
              >
                <div className="mb-auto">
                  <div className={`inline-flex w-12 h-12 rounded-2xl items-center justify-center mb-9 transition ${active ? 'bg-[#B87333] text-white' : 'bg-[#F8F7F4] text-[#B87333]'}`}>
                    <Icon className="w-6 h-6" />
                  </div>
                  <div className="font-display font-semibold text-[23px] tracking-[-0.6px] mb-4 text-[#0B111F]">{p.name}</div>
                  <p className="text-[#0B111F]/65 leading-relaxed text-[15px]">{p.desc}</p>
                </div>
                <div className="mt-8 pt-6 text-[10px] border-t border-[#E6E4DE] tracking-widest text-[#0B111F]/40 group-hover:text-[#B87333] transition">0{idx+1} — CORE DISCIPLINE</div>
              </div>
            );
          })}
        </div>
